<template>
	<div class="influencerCard card">
		<div class="influencerCard__image">
			<img :src="require(`@/assets/images/${influencer.image}`)" class="img-fluid" alt="" />
		</div>
		<div class="influencerCard__detail">
			...
			<div class="d-flex justify-content-between align-items-center">
				<ul>
					<li class="icon">
						<img :src="require(`@/assets/images/svg/${influencer.logo}`)" alt="" class="img-fluid" />
					</li>
					<li>
						{{ influencer.socialMedia }}
						<p>{{ influencer.popularity }}</p>
					</li>
				</ul>
				<router-link to="influencer-detail" class="btn btn-primary">From 1.000 RON</router-link>
			</div>
		</div>
	</div>
</template>
...
.influencerCard {
	...
	&__detail {
		padding: rem(12px) rem(20px) rem(22px);
		&--title {
			font-size: rem(23px);
